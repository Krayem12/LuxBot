...
def process_alerts(alerts):
    for alert in alerts:
        ...
        print(f"📩 Incoming signal for {ticker}: {signal}")
        ...
        if not is_trend_signal and REGULAR_SIGNALS_ENABLED:
            ...
            print(f"📊 Memory for {ticker}: {len(signal_memory[ticker]['bullish'])} bullish, {len(signal_memory[ticker]['bearish'])} bearish")
    ...
    for symbol, signals in list(signal_memory.items()):
        for direction in ["bullish", "bearish"]:
            ...
            if signal_count > 0:
                ...
                print(f"🔍 Checking {symbol} {direction}: {signal_count} signals, {len(unique_signals)} unique, required: {REQUIRED_SIGNALS}")
