...
def process_signal(signal_text: str):
    ...
    if len(signals_store[symbol][direction]) >= 3:
        ...
        # اختيار الرموز والنص حسب الاتجاه
        ...
        # نص الرسالة النهائي
        message = f"{arrow_emoji} {symbol} - {color_emoji} تأكيد إشارة {main_direction_text} قوية\n\n"
        message += "📊 الإشارات المختلفة:\n"
        for sig in signals_list:
            message += f"• {sig}\n"
        message += f"\n🔢 عدد الإشارات الكلي: {total_signals}\n"
        message += f"⏰ التوقيت السعودي: {sa_time}\n\n"
        message += f"{color_emoji} متوقع حركة {main_direction_text} من {total_signals} إشارات مختلفة"
